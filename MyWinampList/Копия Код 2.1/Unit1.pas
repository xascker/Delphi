...
begin
  ...
  N3.Enabled := True;
  N4.Enabled := True;
  spSkinLabel2.Caption := ' PlayList открыт для редактирования ';
end;
...
spSkinLabel2.Caption := ' Добавлено '+ FloatToStr(v)+' файла' ;
...
begin
 GenerateFileList;
 v := ValueListEditor1.Strings.Count;
 spSkinLabel2.Caption := ' Сгенирированно '+ FloatToStr(v)+' файла' ;
end;
...
begin
 if spDynamicSkinForm.AlphaBlend= true then
      N11.Caption:= 'Не прозрачная форма'
   else
 if spDynamicSkinForm.AlphaBlend= False then
     N11.Caption:= 'Полупрозрачная форма'
end;
